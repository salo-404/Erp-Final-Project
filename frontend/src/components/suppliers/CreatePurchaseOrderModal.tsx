...
interface CreatePurchaseOrderModalProps {
  ...
  products: Product[];
  /** Prefills destination + a single item row (e.g. from a Control Tower restock recommendation) — still fully editable. */
  initialWarehouseId?: number;
  initialItem?: { productId: number; quantity: number };
  onClose: () => void;
  onSubmit: (input: CreateIncomingInput) => Promise<void>;
}
...
export function CreatePurchaseOrderModal({ suppliers, warehouses, products, initialWarehouseId, initialItem, onClose, onSubmit }: CreatePurchaseOrderModalProps) {
  const [supplierId, setSupplierId] = useState(suppliers[0]?.id.toString() ?? "");
  const [destinationWarehouseId, setDestinationWarehouseId] = useState(
    initialWarehouseId ? String(initialWarehouseId) : warehouses[0]?.id.toString() ?? "",
  );
  const [expectedDate, setExpectedDate] = useState("");
  const [items, setItems] = useState<ItemRow[]>([
    initialItem
      ? { productId: String(initialItem.productId), quantity: String(initialItem.quantity), price: "" }
      : { productId: products[0]?.id.toString() ?? "", quantity: "", price: "" },
  ]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const parsedItems = items
        ...
        .map((r) => ({ productId: Number(r.productId), quantity: Number(r.quantity), price: Number(r.price || 0) }));
    ...
    <Modal title="Create Purchase Order" onClose={onClose}>
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div>
            <label style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 6, display: "block" }}>Supplier</label>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {items.map((row, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1.6fr 0.7fr 0.8fr auto", gap: 8, alignItems: "center" }}>
                ...
                <input type="number" min={0} step="0.01" placeholder="Price" value={row.price} onChange={(e) => updateItem(i, { price: e.target.value })} style={inputStyle} />
                <button
                  type="button"
