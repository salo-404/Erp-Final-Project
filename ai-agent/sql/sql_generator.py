...
# Can be overridden later in deployment with BEDROCK_SQL_MODEL_ID.
MODEL_ID = os.getenv(
...
""".strip()
    ...
    if not content:
        raise RuntimeError("Nova returned an empty response")
    ...
    if not text_parts:
        raise RuntimeError("Nova did not return SQL text")
