  ...
  async getControlTowerAlerts(
    ...
    for (const entry of deadStock) {
      alerts.push({
        ...
        message:
          entry.lastOutgoingMovementAt === null
            ? `Product ${entry.productId} in warehouse ${entry.warehouseId} has ${entry.onHand} units on hand and has never had an outgoing stock movement`
            : `Product ${entry.productId} in warehouse ${entry.warehouseId} has ${entry.onHand} units on hand with no outgoing stock movement in ${entry.daysSinceLastOutgoingMovement} days`,
