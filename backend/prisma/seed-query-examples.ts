...
`,
...

  {
    ...
    description:
      'Summarize the movement ledger by warehouse and movement type.',
    sql: `
SELECT
    w.id AS warehouse_id,
    w.name AS warehouse_name,
...
`,
...

  {
    ...
    description:
      'Integrity check comparing WarehouseInventory.onHand with the sum of StockMovement quantities.',
    sql: `
SELECT
    p.id AS product_id,
    p.name AS product_name,
    ...
    COALESCE(SUM(sm.quantity), 0) AS ledger_on_hand,
    wi."onHand" - COALESCE(SUM(sm.quantity), 0) AS difference
FROM "WarehouseInventory" wi
JOIN "Product" p
...
GROUP BY
    ...
    w.name,
    wi."onHand"
HAVING wi."onHand" <> COALESCE(SUM(sm.quantity), 0)
ORDER BY ABS(wi."onHand" - COALESCE(SUM(sm.quantity), 0)) DESC
`,
  },
...
`,
...

  {
    ...
    description:
      'Use signed StockMovement quantities to calculate net inventory change.',
    sql: `
SELECT
    p.id AS product_id,
    p.name AS product_name,
    w.id AS warehouse_id,
    w.name AS warehouse_name,
    SUM(sm.quantity) AS net_stock_change
FROM "StockMovement" sm
JOIN "Product" p
    ON p.id = sm."productId"
JOIN "Warehouse" w
