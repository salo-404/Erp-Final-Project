...
export function transactionStatusBadge(status: InventoryTransactionStatus, expectedDate: string | null): StatusBadge {
  if (status === "COMPLETED") {
  ...
  if (expectedDate && new Date(expectedDate).getTime() < Date.now()) {
